...
export default async function OurStoryPage() {
  ...
  try {
    const blocks = await prisma.contentBlock.findMany({
      where: { page: "story", isActive: true },
    });
    blocks.forEach((b) => {
      contentBlocks[b.key] = b;
    });
  } catch (error) {
    console.warn("Using fallback story content:", error);
  }

  const heroBlock = contentBlocks["story_hero"];
  const wageBlock = contentBlocks["story_wage_equity"];
    ...
    <div className="min-h-screen bg-[#FAF8F5] pb-24">
      ...
      <div className="relative bg-charcoal text-white py-24 px-4 sm:px-6 lg:px-8 border-b border-stone-800 text-center overflow-hidden">
        <div className="absolute inset-0 opacity-20">
          <Image
            src={heroBlock?.imageUrl || "https://images.unsplash.com/photo-1600121848594-d8644e57abab?auto=format&fit=crop&w=2000&q=80"}
        ...
        <div className="relative z-10 max-w-4xl mx-auto space-y-4">
          <span className="text-xs uppercase tracking-[0.25em] text-mustard-500 font-semibold block">
            {heroBlock?.subtitle || "Our Provenance"}
          </span>
          <h1 className="font-serif text-3xl sm:text-5xl md:text-6xl font-light leading-tight">
            {heroBlock?.title || "For the People. By the People."}
          </h1>
          <p className="text-stone-300 text-base sm:text-lg max-w-2xl mx-auto font-light leading-relaxed">
            {heroBlock?.content || "PeepalKrat was founded on a simple, uncompromising truth: the women of rural Haryana do not need charity. They possess centuries of inherited craftsmanship that commands reverence on the global stage."}
          </p>
        </div>
      </div>
